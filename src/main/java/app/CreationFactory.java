...
//        String addTermToFile = "./src/main/resources/shellscripts/addTermToFile.sh " + searchTerm;
        String addTermToFile = "ffmpeg -i \"./VAR-Encyclopedia/.temp/combinedImages.mp4\" -vf drawtext=\"fontfile=/path/to/font.ttf: text="+searchTerm+": fontcolor=white: fontsize=24: box=1: boxcolor=black@0.5: boxborderw=5: x=(w-text_w)/2: y=(h-text_h)/2\" -codec:a copy ./VAR-Encyclopedia/.temp/combinedVideo.mp4\n" ;
...
//        String combineVideoAndAudioCommand = "./src/main/resources/shellscripts/combine.sh " + nameOfCreation;
        String combineVideoAndAudioCommand = "ffmpeg -i ./VAR-Encyclopedia/.temp/combinedVideo.mp4 -i ./VAR-Encyclopedia/.temp/tempCombinedChunks.wav -c:a aac -strict experimental ./VAR-Encyclopedia/Creations/"+nameOfCreation+".mp4";
